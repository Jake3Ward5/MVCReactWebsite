...
(async function DeleteFile() {
  ...
  try {
    ...
    await sleep(3000);
    driver.findElement(By.xpath('//*[@id="root"]/div/main/div/div/div[2]/button')).click();

    await sleep(5000);

    let response = await isAlertPresent(driver)
    if(response === "Deleted File"){
        console.log('Assert true. File has been Deleted Safely')
    } else {
        console.log('Assert false. File Failed To be Deleted')
    }
